...
* TODO Speech-to-text
  ...
  #+BEGIN_SRC clojure :tangle parse-and-play.clj :shebang #!/usr/bin/env clj
    ...
    (def fetch-something
      (λ [{what :what
           where :where}]
         #_(fetch :places :q (format "%s,%s" what where) :include_count true)
         (fetch :places :q what :filters {"locality" where} :include_count true)))
    
    (factual! *key* *secret*)
    (let [query "find corsets in los angeles"
          results (fetch-something (parse-query query))]
      (debug (meta results)
             results))
