...
* TODO Speech-to-text
  ...
  #+BEGIN_SRC clojure :tangle parse-and-play.clj :shebang #!/usr/bin/env clj
    ...
    (def consider
      (λ [query]
         (println (format "You: %s" query))
         (let [{what :what where :where} (parse-query query)
               results (fetch-something what where)
               {total :total_row_count
                included :included_rows}
               (:response (meta results))]
           (cond (empty? results)
                 (format "I couldn't find any places for '%s' in %s." what where)
                 (= 1 total)
                 (format "The only place for '%s' in %s appears to be %s."
                         what
                         where
                         (:name (car results)))
                 :else
                 (format "Of the %s or so places for '%s' in %s, you might like %s."
                         total
                         what
                         where
    ...
    ;;; Listen, consider, answer.
    ;; (answer (consider "find corsets in los angeles"))
